...
- After a service-worker restart, `chrome.offscreen.hasDocument()` may still report a document from the previous session whose `onMessage` listener is gone — `sendMessage` then silently has no receiver and the copy fails. `ChromeDelegate.copyToClipboard` therefore always closes any existing offscreen document before creating a new one.
- `chrome.offscreen.createDocument()` resolves on initial page load, not on module-script execution — the offscreen `onMessage` listener may not yet be registered when `createDocument` resolves on a cold path. `ChromeDelegate` works around this with a ready-signal handshake: the offscreen document sends `{ target, type: 'ready' }` immediately after installing its listener, and `ChromeDelegate.ensureOffscreenDocument` waits for that ping (with a 2s timeout) before dispatching the actual `copy` message.
- `defineBackground` calls `ChromeDelegate.prewarmOffscreen()` on every SW wake-up to land `offscreen.html` and its JS chunk in disk cache / V8 code cache. Without this, the first user-initiated copy after a long idle period takes long enough that the omnibox-enter transient activation (~5s) expires before `execCommand('copy')` runs, and the copy silently no-ops while `execCommand` still returns `true`.
- Tailwind v4 makes `<button>` default to `cursor: default`. `src/style.css` adds a base-layer rule that restores `cursor: pointer` for `button:not(:disabled)` and `[role='button']:not([aria-disabled='true'])`. Use `[aria-disabled='true']` rather than `:disabled` for `[role='button']` — the CSS `:disabled` pseudo only matches form-associated elements, so it silently no-ops on `<div role="button">`.
